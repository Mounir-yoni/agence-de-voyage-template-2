...
const ExistingTrips = () => {
    ...
    const handleDelete = async (tripId) => {
        if (window.confirm('Are you sure you want to delete this trip?')) {
            try {
                const token = getLocalStorage('token');
                await axios.delete(`https://back-end-2-e1b4.onrender.com/api/v1/voyages/${tripId}`, {
                    headers: {
                        'Authorization': `Bearer ${token}`
                    }
                });
    ...
    const handleEdit = async (tripId) => {
        ...
        try {
            const token = getLocalStorage('token');
            const response = await axios.get(`https://back-end-2-e1b4.onrender.com/api/v1/voyages/${tripId}`, {
                headers: {
                    'Authorization': `Bearer ${token}`
                }
            });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            console.log(token);
            await axios.put(`https://back-end-2-e1b4.onrender.com/api/v1/voyages/${editingTrip._id}`, jsonData, {
                headers: {
                    'Authorization': `Bearer ${token}`,
